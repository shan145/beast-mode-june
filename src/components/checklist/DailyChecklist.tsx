...
function buildTasks(goals: Goal[], completions: Completion[], today: string): Task[] {
  ...
  return goals.map(goal => {
    ...
    return {
      goal,
      checked: doneToday || quotaMet,
      locked: quotaMet && !doneToday,
      weekCount,
    }
  })
}
...
function fireCelebration() {
  ...
  setTimeout(() => {
    confetti({ ...opts, particleCount: 120, origin: { x: 0.75, y: 0.5 } })
  }, 180)
}

export default function DailyChecklist({ userId, goals, completions }: Props) {
  const today = todayET()
  ...
  async function handleToggle(task: Task, e: React.MouseEvent<HTMLButtonElement>) {
    ...
    // Locked: quota met via other days, no action
    if (task.locked) return

    fireItemConfetti(e)
    await addCompletion(userId, task.goal.id, today)

    // Optimistically check if all tasks will be done after this write
  ...
  if (goals.length === 0) {
    return (
      <div className="text-center py-16 text-gray-500">
        ...
        <p className="text-sm">Add goals in the Goals tab to start tracking.</p>
      </div>
    )
  }
    ...
    <div>
      ...
      {allDone && (
        <div className="mt-6 text-center py-4 rounded-xl bg-emerald-900/30 border border-emerald-800/50">
          <p className="text-emerald-400 font-semibold">All done for today!</p>
          <p className="text-emerald-600 text-sm mt-0.5">Keep the streak going.</p>
        </div>
      )}
    </div>
  )
